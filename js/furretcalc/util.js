...
export function calculate_damage_subtotal(generation, move_data, attacker, stats, is_crit) {
    const attack = stats.attack & 65535
    const defense = stats.defense & 65535
    let level = attacker.data.level & 255

    // TODO: Handle Gen 1 which can divide by 0 here potentially

    if(is_crit && generation === Generation.Gen1) {
        level *= 2
    }
    ...
    if(is_crit && generation === Generation.Gen2) {
        total_damage *= 2
    }
